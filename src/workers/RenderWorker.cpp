...
void RenderWorker::run() {
  ...
  const bool normalizeAudio = property("normalizeAudio").toBool();

  QString targetExt = outputFormat.isEmpty() ? fmtName : outputFormat.toLower();
  if (QFileInfo(m_outputPath).suffix().toLower() != targetExt) {
    const QFileInfo fi(m_outputPath);
    m_outputPath = fi.dir().filePath(fi.completeBaseName() + "." + targetExt);
  }

  const bool sameContainer = (targetExt == fmtName);
  const bool wantsResize = !resolution.isEmpty() && resolution != "source";
  const bool wantsFpsChange = frameRate > 0;
  const bool needsReencode =
      !sameContainer || wantsResize || wantsFpsChange || normalizeAudio;

  emit logOutput(rlogHtml(
      QStringLiteral("Found %1 .%2 clip(s). %3")
          .arg(files.size())
          .arg(fmtName)
          .arg(needsReencode
                   ? QStringLiteral("Re-encoding to .%1 (%2%3%4).")
                         .arg(targetExt,
                              wantsResize ? resolution + "p " : QStringLiteral(),
                              wantsFpsChange ? QString::number(frameRate) + "fps " : QStringLiteral(),
                              normalizeAudio ? QStringLiteral("normalized audio") : QStringLiteral("CRF %1").arg(crf))
                   : QStringLiteral("Using stream-copy (no re-encode).")),
      "#66ccff"));
  ...
  if (!needsReencode) {
    ...
    if (wantsResize)
      args << "-vf" << QStringLiteral("scale=-2:%1").arg(resolution);

    const QString vcodec = videoCodecFor(targetExt, hwAccel);
    args << "-c:v" << vcodec;
    ...
    if (vcodec == "prores_ks") {
      ...
      args << "-crf" << QString::number(crf) << "-preset" << "medium";
    }
    ...
    if (normalizeAudio)
      args << "-af" << "loudnorm=I=-16:TP=-1.5:LRA=11" << "-c:a" << "aac" << "-b:a" << "192k";
    else
      args << "-c:a" << audioCodecFor(targetExt);
  }
